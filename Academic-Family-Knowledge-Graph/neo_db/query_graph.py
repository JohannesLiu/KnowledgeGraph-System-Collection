...
def query(name):
    data = graph.run(
        "match(p )-[r]->(n:Person) where n.name=~'(?i)%s'  return  p.name,r.relation,n.name\
            Union all\
        match(p:Person) -[r]->(n) where p.name=~'(?i)%s' return p.name, r.relation, n.name" % (name, name)
    )
    data = list(data)
...
def get_KGQA_answer(array):
    ...
    for i in range(len(array) - 2):
        ...
        data = graph.run(
            "match(p)-[r:%s{relation: '%s'}]->(n:Person) where n.name=~'(?i)%s' return  p.name,n.name,r.relation, labels(p), labels(n)" % (
